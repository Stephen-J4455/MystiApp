...
if [[ "${SKIP_FUNCTIONS:-0}" != "1" ]]; then
  for fn in health paystack-subaccount send-notification super-agent-offers super-agent-tier-management super-agent-user-management verify-payment reorder-held-agent-order cancel-admin-order verify-wallet-topup get-packages make-orders get-orders check-balance get-order-status; do
