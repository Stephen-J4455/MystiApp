...
on the `EXPO_PUBLIC_APP_ENV` variable.

## Environments

| Env name    | What it is                                       | Paystack mode |
| ----------- | ------------------------------------------------ | ------------- |
| development | `supabase start` locally + Paystack TEST keys    | TEST          |
| test        | Dedicated Supabase TEST project + TEST keys      | TEST          |
| production  | The live Supabase project + LIVE Paystack keys   | LIVE          |

The `APP_ENV` secret is also pushed to each project's edge functions so
the functions themselves know which env they are deployed in (see
`supabase/functions/_shared/env.ts` and `supabase/functions/health/index.ts`).

## One-time setup

1. Install the Supabase CLI: `npm i -g supabase` (or use
   `npx supabase ...`).
2. Create two Supabase projects (one for `test`, one for `production`).
3. In the Supabase dashboard, copy each project's URL, anon key, and
   service-role key.
...
1. `supabase link` to the TEST project.
2. `supabase db push` to apply every migration in `supabase/migrations/`.
3. `supabase functions deploy` for every function under
   `supabase/functions/`.
...
to the TEST project's edge function secret store. Also adds
`APP_ENV=test` so the functions can identify themselves.
...
Calls `<test-project-url>/functions/v1/health` and prints the JSON
response. The health endpoint reports:
...
Production deployments should *always* push fresh secrets first, then
deploy, then verify.
...
# Force a specific env:
npm run env:test    # EXPO_PUBLIC_APP_ENV=test    ? Supabase TEST project
npm run env:prod    # EXPO_PUBLIC_APP_ENV=production ? Supabase LIVE project
```
...
`app.config.js` reads `EXPO_PUBLIC_APP_ENV` from the env file and
exposes it through `extra.appEnv` so `src/lib/env.js` can pick the
right backend.
...
| ----------------------------------------------- | ----------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| `Health check returned non-JSON response`       | The function URL is wrong or the project is offline   | Re-check `EXPO_PUBLIC_SUPABASE_TEST_URL` in `supabase/.env.test`                          |
...
| Mobile app still hits the old project           | Stale build cache or `EXPO_PUBLIC_APP_ENV` not set    | `npx expo start -c` to clear the Metro cache, or pass `--env-file .env.test` to EAS       |
