...
export default function SuperAgentOffersScreen({ navigation }) {
  ...
  const loadData = async () => {
    try {
      ...
      console.error("Error loading super-agent offers:", error);
  ...
  const fetchAgents = async (superAgentId) => {
    try {
      ...
      const assignedAgents = (data?.users || []).filter((member) => {
        const role = member.user_metadata?.role?.toLowerCase?.() || "";
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.card}>
          ...
          {offers.length === 0 ? (
            <Text style={styles.emptyStateText}>
              Your offer list is empty. Create an offer first.
            </Text>
          ) : (
          ...
          <TouchableOpacity
            style={[
              ...
              (!selectedOfferId || !selectedAgentId) && styles.disabledButton,
            ]}
            onPress={handleAssignOffer}
            disabled={!selectedOfferId || !selectedAgentId}
          >
            <Text style={styles.primaryButtonText}>Assign Selected Offer</Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
    </SafeAreaView>
